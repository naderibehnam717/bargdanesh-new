...
import { subjectContent, fileContent } from "@/lib/schoolContent";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const url = `https://www.bargdanesh.ir/school/${file.slug}`;
  const title = `${file.title} — دانلود رایگان`;
  const description = `${file.desc} | دانلود رایگان ${file.type} مدرسه‌ای از برگ دانش`;
...
export default async function SchoolDetailPage({ params }: PageProps) {
  ...
  // ─── محتوا: اول مخصوص فایل، بعد مخصوص دسته ───
  const content =
    fileContent[file.slug] ||
    subjectContent[file.category] || {
      intro:
        "این منبع مدرسه‌ای با هدف کمک به دانش‌آموزان در درک بهتر مفاهیم درسی تهیه شده. محتواش ساده، خلاصه و نکته‌محوره.",
      why: "استفاده از منابع خلاصه و نکته‌محور، یکی از مؤثرترین روش‌های مطالعه برای امتحانات مدرسه‌ست.",
      concerns: [],
    };
